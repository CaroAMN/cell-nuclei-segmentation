# test file for traning without mlf core? 
import pytorch_lightning as pl 
import torch 
import numpy as np 
from argparse import ArgumentParser
from torch.nn import functional as F 
from model.model import UNet2D
from losses.losses import *
from metrics.metrics import iou_fnc, accuracy

class TissueSegmentator(pl.LightningModule):
    def __init__(self, **kwargs):
        """
        Initializes the Network ..
        """
        super(TissueSegmentator, self).__init__()
        ...
        self.model = UNet2D(self.args["n_channels"], self.args["n_class"], dropout_val=self.args["dropout_rate"])
        ...
        parser.add_argument('--val-batch-size', type=int, default=8, help='Input batch size for validation')
    ...
    def forward(self, x):
        ...
        x = self.model(x)
        output = F.softmax(x, dim=1) # das stimmt eher nicht 
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        self.log('train_avg_acc', train_avg_acc, sync_dist=True, on_step=False, on_epoch=True)
    ...
    def test_step(self, test_batch, batch_idx): # batch indices ( brauch ich die wenn ich seperate pfade habe , wahrscheinlich nciht )
        """
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log('test_avg_acc', test_avg_acc, sync_dist=True, on_step=False, on_epoch=True)
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log('val_avg_acc', test_avg_acc, sync_dist=True, on_step=False, on_epoch=True)
